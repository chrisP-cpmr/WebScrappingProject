...
def scrappeHitparade():
    ...
    since = int(input("Since when (year) do you want to scrappe the data? (format: yyyy) "))   # User can define since when to scape
    ...
    filename = "../Meier_Christoph_Data/Hitparade.ch_" + str(since) + "_test.csv"
    f = open(filename, "w")

    # create and write headers
    fileHeaders = "VW, ArtistAndTitle, WeeksAndPeak, Rank, Week\n"
    f.write(fileHeaders)
    ...
    for week in weeks:
        ...
        for chartItem in page_soup.find_all("div", class_="content chartitem"):     # get the list of chart items
            ...
            str_ArtistAndTitle = str(chartItem.find("a"))  # get the <a> tag and save it as string
            # this step is done because, if you directly save the .getText() output, the names will be concatenated an
            # are difficult to split. As a result, we save it as string and save the result after the first <b>
            ArtistAndTitle = str_ArtistAndTitle.split("<b>")[1]  # split the string and save
            WeeksAndPeak = chartItem.find("span", class_="wp").getText().strip()   # retrieve the value for WeeksAndRank
            Rank = chartItem.find("div", class_="chart_pos").getText().strip()  # retrieve the value for Rank
            # we have to replace all comas in artist names or titles to ensure no wrong spacing in the .csv
            f.write(VW + "," + ArtistAndTitle.replace(",", "|") + "," + WeeksAndPeak + "," + Rank + "," + week + "\n")
